...
path1 = "../Documents/Books2018/Mac6914/train_data_p1/"
path2 = "train_data_p2/"
path3 = "train_data_p3/"
...
class DataAug(Sequence):
    ...
    def __init__(self, x, y, batch_size):
        self.x = x
        self.y = y
        self.xmod = x[:batch_size]
        self.ymod = y[:batch_size]
    ...
    def on_epoch_end(self):
        ...
        self.xmod = []
        self.ymod = []
        chosen = np.random.choice(np.arange(len(self.x)), batch_size, replace=False)
        for i in chosen:
            if np.random.uniform() > 0.5:
                img, label = produceRandomlyTranslatedImage(self.x[i], self.y[i])
            elif np.random.uniform() > 0.5:
                img, label = produceRandomlyDeformedImage(self.x[i], self.y[i])
            else: 
                img = self.x[i]
                label = self.y[i]
            self.xmod += [img]
...
model.fit(new_imags[:,:,:, :, :], new_labels[:, :, :, :,:], batch_size=2, epochs=30000, verbose=1, callbacks=callback_list, shuffle=True)
